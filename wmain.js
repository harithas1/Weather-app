...
document.addEventListener("DOMContentLoaded",() => {
    const loadCurrentForecast = async ({lat, lon}) =>{   //1

        const cityName = "Chittoor"
        const response = lat && lon ? await fetch(`${END_POINTS.CURRENT_FORECAST_CORD}?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric`): await fetch(`${END_POINTS.CURRENT_FORECAST}?q=${cityName}&appid=${API_KEY}&units=metric`)
    ...
    const getHourlyForecast = async (city)=>{
        const response = await fetch(`${END_POINTS.FIVE_DAY_FORECAST}?q=${city}&appid=${API_KEY}&units=metric`);
    ...
    // 3
    const createCurrentForecast = ({ sys:{country, sunrise, sunset}, name, main: {temp, temp_min, temp_max}, weather:[{description,icon}] })=>{  //3
        // <h1>name</h1>
        // <p class="temp">24</p>
        ...
        minMaxTemp.textContent = `Max: ${formatTemp(temp_max)} Min: ${formatTemp(temp_min)}`
    ...
    const createFiveDayForecast = (hourlyForecast)=>{   //7
    ...
    const getGeoLocationInfo = async (cityName) => {
        const response = await fetch(`${END_POINTS.GEO_LOCATION}?q=${cityName}&limit=5&appid=${API_KEY}`)
        return response.json();
